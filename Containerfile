...
RUN curl -o /root/go1.17.8.linux-amd64.tar.gz -L https://go.dev/dl/go1.17.8.linux-amd64.tar.gz
WORKDIR /root
RUN tar zxf go1.17.8.linux-amd64.tar.gz
...
RUN go get github.com/miekg/coredns-git && go generate && GOOS=linux GOARCH=amd64 go build
...
COPY --from=builder /lib64/libcrypto.so.1.1 /lib64/libcrypto.so.1.1
COPY --from=builder /lib64/libcrypt.so.1 /lib64/libcrypt.so.1
